...
class UserGroupControllerSpec extends Specification {
    ...
    void "Test the save action correctly persists an instance"() {
        ...
        when:"The save action is executed with a valid instance"
            ...
            userGroup = new UserGroup(params)

            controller.save(userGroup)

        then:"A redirect is issued to the show action"
            response.redirectedUrl == '/userGroup/show/1'
    ...
    void "Test the update action performs an update on a valid domain instance"() {
        ...
        then:"A 404 error is returned"
            response.redirectedUrl == '/userGroup/index'
            flash.message != null
        ...
        when:"A valid domain instance is passed to the update action"
            response.reset()
            populateValidParams(params)
            userGroup = new UserGroup(params).save(flush: true)
            controller.update(userGroup)

        then:"A redirect is issued to the show action"
            userGroup != null
            response.redirectedUrl == "/userGroup/show/$userGroup.id"
            flash.message != null
    }
    ...
    void "Test that the delete action deletes an instance if it exists"() {
        ...
        when:"A domain instance is created"
            response.reset()
            populateValidParams(params)
            def userGroup = new UserGroup(params).save(flush: true)
